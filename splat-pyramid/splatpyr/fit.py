...
class FitConfig:
    ...
    loss_aware: float = 0.1     # detail levels: dropout penalty, so a lost packet leaves little mark
...
def fit_unit(target, prediction, margin, w, h, cfg, base, inside=None):
    ...
    if not base and np.mean(goal[inner] ** 2) <= target_mse:
        ...
        # Detail levels only. The base is a weighted average of its blobs, which the penalty
        # fights (the top unit of bills.jpg fell from 45 to 30 dB); the protocol repairs the
        # base unit fully instead.
        loss_aware = 0.0 if base else cfg.loss_aware
        ...
        def attempt(layout_mse):
            ...
            for round_ in range(cfg.rounds):
                ...
                b = solve_colours(b, goal, row_w, base, cfg.damping, loss_aware)
                if base:
            ...
            shaped = solve_colours(shaped, goal, row_w, base, cfg.damping, loss_aware)
...
def solve_colours(blobs, goal, row_w, normalized, damping, loss_aware=0.0):
    ...
    loss_aware > 0 fits for a link that loses packets. Losing packets drops blobs at
    random, which is dropout; for a linear model dropout's expected error adds a penalty on
    each blob in proportion to its own energy (the diagonal of A^T A). No blob may then carry
    more than its share, so a lost one leaves a slight softness instead of a visible mark.
    Measured on bills.jpg level 2 at 0.1: -0.45 dB with no loss, damage at 5% loss halved
    (34.2 dB instead of 30.7)."""
    Hp, Wp = goal.shape[:2]
    n = len(blobs)
    if n == 0:
        return blobs
    ...
    reg = lam * sp.identity(n, format="csc") + loss_aware * sp.diags(AtA.diagonal())
    solve = factorized((AtA + reg).tocsc())
    rhs = A.T @ t + lam * prior
